...
abstract public class UniqController extends HttpServlet {
          ...
          case PARAMETER:
            ...
            if (value == null) {
              if (parameters.get(parameterIndex).required()) {
                final mg.razherana.uniqcontrol.annotations.Parameter parameter = parameters.get(parameterIndex);
                return (re, rs, rp) -> {
  ...
  private Route all(String path, String routeMethodName, RouteInterface defaultRoute,
    ...
    return all(path, (req, res, all) -> routeMethod.makeInterface(req, res, all).handle(req, res, all), defaultRoute,
        container);
  }
  ...
  final protected void redirect(String path)
    ...
    globalResponse.sendRedirect(
        globalRequest.getContextPath().trim().replaceAll("/+$", "") + "/" + path.trim().replaceAll("^/|/$", ""));
  }
   ...
   */
  abstract protected String[] routeBase();
